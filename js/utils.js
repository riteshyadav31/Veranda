...
export function renderState(container, { title, message, tone = "empty" }) {
  if (!container) return;
  container.innerHTML = `
...
const HOUSE_ICON =
  ...
  '<path d="M3 10.5 12 3l9 7.5"/><path d="M5 9.8V21h14V9.8"/><path d="M10 21v-6h4v6"/></svg>';

/** Markup for the block that stands in for a missing photograph. */
export const PHOTO_PLACEHOLDER = `<div class="ph" aria-hidden="true">${HOUSE_ICON}</div>`;

export const typeLabel = (value) => PROPERTY_TYPES[value] || "Property";
export const listingLabel = (value) => LISTING_TYPES[value] || "Listed";
...
export function priceLabel(property) {
  ...
  return property.listingType === "rent" ? `${amount} <small>per month</small>` : amount;
}
...
export function photoMarkup(url, alt) {
  ...
  return `<img src="${escapeHtml(url)}" alt="${escapeHtml(alt)}" loading="lazy" data-photo />`;
}
  ...
  qsa("img[data-photo]", scope).forEach((img) => {
    img.addEventListener(
      ...
      () => {
        ...
        if (host) host.innerHTML = PHOTO_PLACEHOLDER;
      },
      { once: true }
    );
  });
...
/** One property card. Used on the home page and the properties listing. */
export function propertyCardHTML(property) {
  const href = `property-details.html?id=${encodeURIComponent(property.id)}`;
  const isPlot = property.propertyType === "plot" || property.propertyType === "commercial";

  const rooms = isPlot
    ? ""
    : `<span><strong>${escapeHtml(property.bedrooms ?? "—")}</strong> beds</span>
         <span><strong>${escapeHtml(property.bathrooms ?? "—")}</strong> baths</span>`;

  return `
    <article class="property-card">
      <div class="property-card__media">
        ${photoMarkup(primaryImage(property), property.title || "Property photo")}
        <span class="tag${property.listingType === "rent" ? " tag--brass" : ""}">${listingLabel(property.listingType)}</span>
      </div>
      <div class="property-card__body">
        <p class="property-card__price">${priceLabel(property)}</p>
        <h3 class="property-card__title"><a href="${href}">${escapeHtml(property.title || "Untitled listing")}</a></h3>
        <p class="property-card__address">${escapeHtml(property.location || "Location not given")} &middot; ${typeLabel(property.propertyType)}</p>
        <p class="meta">
          ${rooms}
          <span><strong>${escapeHtml(formatArea(property.area))}</strong></span>
        </p>
        <a class="btn btn--ghost btn--sm property-card__action" href="${href}">View details</a>
      </div>
    </article>`;
}

/** Render a list of properties into a container, with an empty state. */
export function renderPropertyCards(container, properties, emptyState) {
  if (!container) return;
...
export function renderLoading(container, message = "Loading listings…") {
  renderState(container, { title: "One moment", message, tone: "loading" });
}
